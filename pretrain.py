...
optimizer_config = dict(
    ...
    weight_decay=weight_decay, betas=betas, learning_rate=learning_rate)
...
for step in range(max_steps):
    ...
    if step % eval_interval == 0 or last_step:
        # TODO: put this in eval function (separate file or even class method)?
        model.eval()
        val_loader.reset()
        with torch.no_grad():
            val_loss_accum = 0.0
        ...
        if master_process:
            ...
            if wandb_log:
                try:
                    wandb.log(
                        {
                            # "step": step,
                            # "tokens": step * grad_accum_steps * ddp_world_size * B * T,
                            # "loss/train": loss_accum.item(),
                            "loss/val": val_loss_accum.item(),
                            # "norm": norm,
                            # TODO: log gradient norm separated across layers
                            # "lr": lr,
            ...
            if step > 0 and (step % 5000 == 0 or last_step): # save_interval = 5000
                # optionally write model checkpoints
                checkpoint_path = os.path.join(log_dir, f"model_{step:05d}.pt")
                checkpoint = {
                    'model': raw_model.state_dict(),
                    'config': model_config, # raw_model.config, # TODO: add config class? rather than dict. allow single input
                    'step': step,
    ...
    if (step % 250 == 0 or last_step) and (not use_compile): # TODO: fix hellaswag issue
        num_correct_norm = 0
        num_total = 0
    ...
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # TODO: log norms per layer
